# Talos Battery Benchmark

Reproducible methodology for comparing Talos's energy use against Arc, Brave,
...
## Why Talos should win

- **System WebKit.** Talos rides the same engine processes as Safari — Apple's
...
7. For the hibernation story, run a second Talos pass after the tabs have been
   idle 20+ minutes (`talos-hibernated`) — that's the headline delta.
...
2. Run for 40 minutes: `./memory-bench.sh talos 'Talos|com.apple.WebKit' 2400`
   (same process patterns as the battery script).
3. The `final_mb` column is the headline: after the 15-minute idle threshold,
   Talos's background tabs give up their WebContent processes while Chromium
...
> Talos (my WebKit browser) vs Arc vs Brave vs Zen — identical 12 tabs,
