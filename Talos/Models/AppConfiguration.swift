...
enum BrowserDefaults {
    ...
    static let defaultHomeTitle = "Talos"
...
enum TabHibernationConfiguration {
    /// Background tabs are never torn down on a timer. Like Safari, Talos
...
enum SignInRedirectConfiguration {
    ...
    /// handshake and move on before Talos calls it a dead end. Generous on
    ...
    static var deadEndGrace: TimeInterval {
        ProcessInfo.processInfo.environment["TALOS_UI_TESTING"] == "1" ? 2 : 8
    }
}
...
enum WebInspectorConfiguration {
    ...
    #else
    static let isEnabled = UserDefaults.standard.bool(forKey: "TalosEnableWebInspector")
    #endif
}
...
enum DeveloperModeConfiguration {
    static let storageKey = "Talos.DeveloperModeSiteOverrides"
