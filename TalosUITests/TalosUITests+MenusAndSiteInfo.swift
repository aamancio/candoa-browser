...
extension TalosUITests {
    ...
    /// window.open (OAuth sign-in popups, target=_blank) hands Talos the
    ...
    func testWindowOpenPopupOpensTabWithoutCrashing() {
        ...
        XCTAssertTrue(
            waitForState(in: app, containing: "url=https://fixture.talos.test/popup-child", timeout: 10),
            currentState(in: app)
        )
        ...
        XCTAssertEqual(
            ...
            "https://fixture.talos.test/popup-child",
            currentState(in: app)
        )
    }
    ...
    func testCommandClickWindowOpenStaysInBackground() {
        ...
        XCTAssertTrue(
            waitForState(
                in: app,
                containing: "background href=https://fixture.talos.test/popup-child",
                timeout: 10
            ),
            currentState(in: app)
        )
        XCTAssertEqual(stateValue("active", in: app), "popup", currentState(in: app))
        XCTAssertEqual(stateValue("url", in: app), "https://fixture.talos.test/popup", currentState(in: app))
    ...
    func testStrandedSignInRedirectShowsRecovery() {
        ...
        openNewTabPalette(in: app)
        submitCommandPaletteText("https://fixture.talos.test/redirect#code=FAKE&state=abc", in: app)
    ...
    func testSiteInfoBlocksPopupsForSite() {
        ...
        XCTAssertTrue(
            waitForState(
                in: app,
                containing: "blocked href=https://fixture.talos.test/popup-child",
                timeout: 10
            ),
            currentState(in: app)
        )
    ...
    /// Talos ▸ Site Info presents the same popover from the menu bar, in the
    ...
    func testSiteInfoMenuCommandOpensPopover() {
        ...
        let appMenu = app.menuBars.menuBarItems["Talos"]
    ...
    /// Talos ▸ Privacy Report presents the report sheet: the status row
    ...
    func testHistoryMenuRowsSurviveMidOpenRebuild() {
        ...
        openNewTabPalette(in: app)
        submitCommandPaletteText("https://fixture.talos.test/history-menu-churn", in: app)
        XCTAssertTrue(
            waitForState(in: app, containing: "url=https://fixture.talos.test/history-menu-churn", timeout: 10),
            currentState(in: app)
        )
    ...
    func testOpenLocalFileCommandOpensChosenFile() throws {
        ...
        let app = launchApp(extraLaunchEnvironment: [
            "TALOS_UI_TESTING_OPEN_FILE_FIXTURE": "1",
        ])
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 10))
    ...
    func testSaveAsAndExportAsPDFWriteDocuments() throws {
        ...
        let app = launchApp(
            fixture: "popup-open",
            extraLaunchEnvironment: ["TALOS_UI_TESTING_EXPORT_TO_DOWNLOADS": "1"]
        )
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 10))
        openFixtureTab(path: "saved-page", in: app)
        XCTAssertTrue(waitForState(in: app, containing: "loading=false"), currentState(in: app))
    ...
    /// The Develop menu mirrors Safari's order with Talos's own items kept:
    ...
    func testDevelopMenuOffersSafariParityCommands() throws {
        ...
        // Safari has no Developer Mode row, so neither does Talos's Develop
        ...
        // The device-targets submenu leads with a disabled Talos header,
        // then one enabled row per inspectable page named host — path.
        // Clicking the parent swaps out the Open Page With submenu.
        deviceItem.click()
        // Scoped to the submenu: the Window menu also carries a "Talos" row
        // (the main window's title) in the closed-menu accessibility tree.
        let talosHeaderItem = deviceItem.menuItems["Talos"]
        XCTAssertTrue(talosHeaderItem.waitForExistence(timeout: 3))
        XCTAssertFalse(talosHeaderItem.isEnabled, "the app header row is informational only")
        let inspectablePageItem = deviceItem.menuItems["fixture.talos.test — develop"]
    ...
    func testHostedWebAuthenticationCompletesOnMatchingCallback() throws {
        ...
        XCTAssertTrue(
            waitForState(
                in: app,
                containing: "t1:resolved-completed:talos-e2e://auth?code=ok",
                timeout: 10
            ),
            currentState(in: app)
        )
        ...
        let authWindow = app.windows["Sign In — fixture.talos.test"]
    ...
    func testDockedWebInspectorStaysInsideThePageCard() throws {
        let app = launchApp(
            fixture: "popup-open",
            extraLaunchEnvironment: ["TALOS_UI_TESTING_DOCK_INSPECTOR": "1"]
        )
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 10))
    ...
    private func beginWebAuthRequest(id: String, path: String, mode: String) {
        DistributedNotificationCenter.default().postNotificationName(
            ...
            object: "\(id)|https://fixture.talos.test/\(path)|talos-e2e|\(mode)",
            userInfo: nil,
            deliverImmediately: true
        )
    }
